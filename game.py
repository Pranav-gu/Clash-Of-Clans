...
from characters import spawnBarbarian, move_barbarians, spawnArcher, move_archers, spawnDragon, move_dragons, spawnBalloon, move_balloons, clearTroops, spawnStealthArcher, move_stealth_archers, stealth_archers, spawnHealers, move_healers, king_transfer
...
while(True):
    ...
    if (len(stealth_archers) > 0):
        for i in range(0, len(stealth_archers)):
            # print(f"stealth_archers[{i}].invisibility_time = ", stealth_archers[i].invisibility_time, "stealth_archers[{i}].health = ", stealth_archers[i].health)
            x = time.time()
            difference = stealth_archers[i].creation_time-x
            stealth_archers[i].creation_time = x
            stealth_archers[i].invisibility_time += difference
    ...
    elif ch == 't':                                     # code added
        spawnStealthArcher(V.spawn_points[0])
    elif ch == 'y':                                     # code added
        spawnStealthArcher(V.spawn_points[1])
    elif ch == 'u':                                     # code added
        spawnStealthArcher(V.spawn_points[2])
    elif ch == 'e':                                     # code added
        spawnHealers(V.spawn_points[0])
    elif ch == 'f':                                     # code added
        spawnHealers(V.spawn_points[1])
    elif ch == 'g':                                     # code added
        spawnHealers(V.spawn_points[2])
    elif ch == 'q':
        print('quit')
        break
    # os.system('clear')
    ...
    move_dragons(V)
    move_balloons(V)
    move_stealth_archers(V, pt.movement)                # code added
    move_healers(V, King)                                     # code added
